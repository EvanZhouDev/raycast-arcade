...
function generateRow(rows) {
    ...
    for (let [i, { letter, color }] of Object.entries(rows)) {
        if (letter === "") {
            ...
            <text x="${width * i + spacing * i + width / 2}" y="${width - width / 6}" text-anchor="middle" alignment-baseline="middle" class="large" font-size="300" font-weight="bold" fill="white" font-family="Helvetica Neue">${letter}</text>
            `
        }
    }
...
export default function Wordle() {
    ...
    let [board, setBoard] = useState(Array(6).fill().map(_ => Array(5).fill().map(_ => ({
        letter: "",
        color: Color.GRAY,
    }))))
    ...
    useFetch(`https://www.nytimes.com/svc/wordle/v2/${currentDate}.json`, {
        onData: async (data) => {
            ...
            if (storeDate === currentDate) {
                ...
                (async () => {
                    await LocalStorage.setItem("wordleBoard", JSON.stringify(Array(6).fill().map(_ => Array(5).fill().map(_ => ({
                        letter: "",
                        color: Color.GRAY,
                    })))));
    ...
    const generateBoard = (board) => {
        if (loading) board = Array(6).fill().map(_ => Array(5).fill().map(_ => ({
            letter: "",
            color: Color.GRAY,
        })))
    ...
    >
        <List.Item
            ...
            actions={
                <ActionPanel>
                    {(guessCount.current === 6 || hasGuessed.current) ? <Action
                        title="Share"
                        onAction={share}
                    /> : <Action
                        title="Submit Guess"
